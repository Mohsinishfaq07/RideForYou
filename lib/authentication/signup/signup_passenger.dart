...
import 'package:ride_4_you/global_custom_widgets/custom_medium_button.dart'; // Import the custom_medium_button.dart file for custom medium button
...
class SignUpPassenger extends StatefulWidget { // Define a widget for signing up as a passenger
  const SignUpPassenger({Key? key}) : super(key: key); // Constructor for the sign-up passenger widget
  static const tag = "SignupPassenger"; // Tag for identifying the sign-up passenger screen

  @override
  _SignUpPassengerState createState() => _SignUpPassengerState(); // Create state for the sign-up passenger widget
}

class _SignUpPassengerState extends State<SignUpPassenger> { // Define the state for the sign-up passenger screen
  ...
  final TextEditingController _employeidController = TextEditingController(); // Controller for the employee ID input field

  @override
  Widget build(BuildContext context) { // Build method to create the sign-up passenger screen widget
        ...
        child: SingleChildScrollView( // SingleChildScrollView to allow scrolling when keyboard is open
          ...
          child: Column( // Column to arrange content vertically
            ...
            children: [ // List of children widgets
                  ...
                  Radio<String>( // Radio widget for selecting teacher role
                    ...
                    onChanged: (value) { // Define action when value changes
                      if (value != null) { // Check if value is not null
                        setState(() { // Set state to update UI
                          _role = value; // Update role value
                          _employeidController.clear(); // Clear employee ID controller
              ...
              SizedBox(height: MediaQuery.of(context).size.height * 0.08.h), // SizedBox widget for adding vertical space
              CustomLargeButton( // Custom large button for sign up
                  ...
                  onTap: () { // Define action on button tap
                    if (_usernameController.text.isEmpty || // Check if username is empty
                      ...
                      database.passengerDetailsToFirestore( // Add passenger details to Firestore
      ...
      builder: (BuildContext context) { // Builder for dialog
        return AlertDialog( // AlertDialog widget for teacher registration
          ...
          content: CustomTextField( // Custom text field for employee ID input
            controller: _employeidController, // Employee ID controller
            hintText: 'Enter Employee Id', // Hint text for input field
          ),
          actions: [ // List of actions
            CustomMediumButton( // Custom medium button for OK
              title: 'OK', // Button title
              ontap: () { // Define action on button tap
                _registrationNumber = _employeidController.text; // Update registration number
                Navigator.of(context).pop(); // Close dialog
              },
            ),
          ],
